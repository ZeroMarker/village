...
# db_connection_str = 'mysql+pymysql://a8qt8rd4wvt45smi4mhg:pscale_pw_4EHK1K2CsxadO1CHH3kOI3G00FvODIrBnfrcLkr1anq@aws.connect.psdb.cloud:3306/zavi?sslMode=VERIFY_IDENTITY'
...
df_new['network'] = df[['broadband', 'mobile', 'cable']].sum(axis=1).round(4)
df_new['security'] = df[['data', 'emergency', 'technology']].sum(axis=1).round(4)
df_new['ecosystem'] = df[['soil', 'water', 'air']].sum(axis=1).round(4)
df_new['service'] = df[['government', 'medical', 'online', 'education']].sum(axis=1).round(4)
df_new['industry'] = df[['agriculture', 'traffic', 'irrigation', 'power', 'logistics']].sum(axis=1).round(4)
df_new['overall'] = df.sum(axis=1).round(4)
