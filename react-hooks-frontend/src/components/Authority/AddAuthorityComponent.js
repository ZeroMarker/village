...
const AddAuthorityComponent = ({setShowAddAuthority,getAllAuthoritys}) => {
    ...
    const saveAuthority = (e)=>{
        e.preventDefault();

        const authority = {authorityName,authorityDept,authorityRole,authorityInfo,authorityState};

        // console.log(authority)

        
        AuthoritysService.createAuthority(authority).then(response=>{
           ...
           <div className="container">
               <div className="row">
                   <div className="card col-md-6 offset-md-3 offset-md-3" style={{boxShadow:"1px 2px 2px grey,-1px -2px 2px grey"}}>
                      <div style={{textAlign:"center",fontSize:"20px",fontWeight:"bold"}}> 添加</div>
                           ...
                           <form>
                               ...
                               <div className="form-group mb-2">
                                   <label className="form-label">权限描述 :</label>
                                     {/* <div style={{width:"50%"}}>
                                        <select class="selectpicker " multiple data-max-options="2" id="sel1"  name="authorityInfo" value = {authorityInfo}  onChange={e=>setAuthorityInfo(e.target.value)}>
                                            <option>请选择权限内容</option>
                                            <option>用户管理</option>
	                                        <option>产品管理</option>
    		                                <option>文件管理</option>
                                            <option>部门管理</option>
                                        ...
                                        <form action="/action_page.php" >
                                            ...
                                            <div class="form-check" value = {authorityInfo} onChange={e=>setAuthorityInfo(e.target.value)}>
                                                <input type="checkbox" class="form-check-input" id="check2" name="option2" value="评价管理"></input>
                                                <label class="form-check-label" for="check2">评价管理</label>
                                            </div>
                                            <div class="form-check" value = {authorityInfo} onChange={e=>setAuthorityInfo(e.target.value)}>
                                                <input type="checkbox" class="form-check-input" id="check3" name="option3" value="文件管理"></input>
                                                <label class="form-check-label" for="check2">文件管理</label>
                                            </div>
                                            <div class="form-check" value = {authorityInfo} onChange={e=>setAuthorityInfo(e.target.value)}>
                               ...
                               <div className="FlavorForm form-group mb-2">
                                ...
                                <button className="btn btn-success" onClick={(e)=>saveAuthority(e)}>提交</button>
                                <button style={{marginLeft:"20px"}} className="btn btn-danger" onClick={()=>setShowAddAuthority(false)}>取消</button>
                           </form>
                       </div>
                   </div>
               </div>
...
export default AddAuthorityComponent
